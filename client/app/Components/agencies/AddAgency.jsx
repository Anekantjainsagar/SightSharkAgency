          ...
          <div className="min-[1600px]:h-[48vh] min-[1750px]:h-[52vh] h-[35vh] md:h-[42vh]">
            {page === 1 ? (
              <div className="px-[4vw] min-[1600px]:px-[8vw] w-full">
                ...
                <div className="grid grid-cols-2 gap-x-4 md:gap-x-6 min-[1600px]:gap-x-8 gap-y-3 md:gap-y-4 min-[1600px]:gap-y-6">
...
min-[1600px]:h-[45px] h-[35px] border border-gray-500/20 text-[13px] min-[1600px]:text-base px-4 py-2 rounded-md"
                ...
                <div className="grid grid-cols-1 gap-x-6 min-[1600px]:gap-x-8 gap-y-4 min-[1600px]:gap-y-6">
                  <div className="flex flex-col">
                    ...
                    >
                      Portal Owner Email
                      <Info
                        text="This user will have the Superadmin access on the Client Portal"
                        placement={"bottom"}
                      />
                      <Required />
                    </label>
          ...
          <div className="border-t border-t-gray-100/30 px-[3vw] min-[1600px]:px-[5vw] w-full flex items-center justify-between py-3 md:py-6 mt-10 mainText20">
            <button
              className={`text-white text-[13px] min-[1600px]:text-lg w-[120px] md:w-[150px] min-[1600px]:w-[170px] ${
                page == 1 ? "bg-[#898989]/15" : "bg-newBlue cursor-pointer"
              } h-9 md:h-10 min-[1600px]:h-12 rounded-lg`}
              disabled={page == 1}
              onClick={() => {
                if (page == 4) {
                  setCredentialsState([]);
      ...
      <Accordion>
        {credentialsState?.map((e, i) => (
          <AccordionItem
            ...
            key={i}
            header={({ state }) => (
              <div className="w-[100%] flex flex-row justify-between items-center border-2 border-gray-500/10 bg-[#171C2A]/50 px-3 p-3 rounded-tl-lg rounded-tr-lg">
                <div className={`flex items-center`}>
                  <Image
                    src={e?.img_link}
                    alt={e?.platform}
          ...
          >
            <div
              key={i}
              className="border-b-2 border-l-2 border-r-2 gap-2 border-gray-500/10 px-3 bg-[#171C2A]/50 md:pt-0 pt-3 flex w-full h-full flex-col md:flex-row justify-between items-center rounded-bl-lg rounded-br-lg"
            >
              <div
